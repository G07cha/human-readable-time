		...
		return function(dateOrPattern, pattrn) {
			...
			if(dateOrPattern) {
				if(dateOrPattern instanceof Date) {
					...
					throw new Error('Unknow type of first argument, expected Date or String but recieved <' + typeof dateOrPattern + '>');
				}
			} else {
...
var validate = {
	date: function(date) {
		if(date instanceof Date === false) {
			throw new Error('Provided argument is not instance of Date');
		} else {
			return true;
		}
	},
